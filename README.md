...
Во время работы скрипт взаимодействует с указанной Телеграм-группой и при этом:
...
- трансляция в Телеграм сообщений о подключении/отключении беспроводных клиентов производится скриптом только в случаях, когда МАС-адрес клиента в строке журнала:
    - не найден
    - найден, но ему не прописан соответствующий IP-адрес в DHCP/Leases (т.е. клиент с динамическим IP-адресом)
    - найден, ему назначен IP-адрес в DHCP/Leases (т.е. клиент со статическим IP-адресом), но комментарий в DHCP/Leases отсутствует.
