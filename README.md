...
при этом скрипт должен работать на всех роутерах, которым может быть адресовано сообщение. Для отправки команды конкретному роутеру в ТГ-группе необходимо сформировать текстовое сообщение специальным образом: 
 - начало сообщения обозначается символом "/"
 - указывается имя устройства, которому адресовано сообщение (соответствует /system identity устройства). Для отправки всем роутерам в Телеграм-группе в качестве адресата указать: forall. Для корректной работы скрипта имя устройства должно состоять ТОЛЬКО из цифр и маленьких латинских букв (!!!).
 - через символ подчеркивания или пробел указывается непосредственно команда. В качестве команды могут выступать: имя глобальной функции, имя скрипта или команда RouterOS.
